...
void createSmallExplosion(Vector3 center) {
    rbExplosionFAux = new ExplosionRBForceGenerator(center, 300.0, 1499999999.0, 0.3);
...
>>>>>>> Stashed changes
    ...
    case '9':
        createSmallExplosion(Vector3(400, 15, 0));
        break;

    case ' ':
        levelManager->birdInteract();
...
void onCollision(physx::PxActor* actor1, physx::PxActor* actor2)
{
    ...
    AngryBirdsObject* obj1 = levelManager->getObjFromActor(actor1);
    AngryBirdsObject* obj2 = levelManager->getObjFromActor(actor2);

    if (obj1 == nullptr || obj2 == nullptr) return;

    if (obj1->hurtsOthers() && actor2->is<PxRigidDynamic>()) {
        if (actor1->is<PxRigidDynamic>()) {
            ...
            obj2->hurt(static_cast<PxRigidDynamic*>(actor2)->getLinearVelocity());
        }
    }

    if (obj2->hurtsOthers() && actor1->is<PxRigidDynamic>()) {
        if (actor2->is<PxRigidDynamic>()) {
            ...
            obj1->hurt(static_cast<PxRigidDynamic*>(actor1)->getLinearVelocity());
        }
    }
}
